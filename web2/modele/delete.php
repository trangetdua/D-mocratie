...
try {
    ...
    $deleteUrl = "https://projets.iut-orsay.fr/saes3-aviau/TestProket/web2/controller/api.php/{$table}?method=DELETE&key={$key}&value={$value}";
    ...
    if (isset($resultDelete['message']) && strpos($resultDelete['message'], 'supprimé') !== false) {
        session_destroy();
        header("Location: ../vue/connection.php?message=deleted");
        exit;
    } else {
        echo "<pre>API Response: " . htmlspecialchars(json_encode($resultDelete)) . "</pre>";
